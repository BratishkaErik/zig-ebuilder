...
fn prepare(
    ...
    // Synced with Zig build runner, version "0.16.0-dev.2276+c857fce05".
    if (true) return zig_ebuilder_section: {
        const Report = @import("Report.zig");

        var system_libraries: std.StringArrayHashMapUnmanaged([][]const u8) = .empty;
        system_libraries.ensureUnusedCapacity(gpa, step_stack.count() + b.modules.count()) catch @panic("OOM");

        for (step_stack.keys()) |s| {
            const artifact = Step.cast(s, Step.Compile) orelse continue;
            const link_objects = artifact.root_module.link_objects.items;
            system_libraries.ensureUnusedCapacity(gpa, link_objects.len) catch @panic("OOM");
            for (link_objects) |obj| switch (obj) {
                .system_lib => |lib| {
                    ...
                    if (gop.found_existing == false) {
                        ...
                        var names: std.ArrayList([]const u8) = .fromOwnedSlice(old);
        ...
        for (b.modules.keys(), b.modules.values()) |module_name, module| {
            const link_objects = module.link_objects.items;
            system_libraries.ensureUnusedCapacity(gpa, link_objects.len) catch @panic("OOM");
            for (link_objects) |obj| switch (obj) {
                .system_lib => |lib| {
                    const gop = system_libraries.getOrPutAssumeCapacity(lib.name);
        ...
        var user_options: std.ArrayList(Report.UserOption) = .empty;
        user_options.ensureUnusedCapacity(gpa, b.available_options_list.items.len) catch @panic("OOM");
        for (b.available_options_list.items) |option| user_options.appendAssumeCapacity(.{
            .name = option.name,
            .description = option.description,
            .type = switch (option.type_id) {
                .@"enum" => "enum",
                ...
                .float => "float",
                .lazy_path => "LazyPath",
                .lazy_path_list => "list of LazyPath",
                .string => "string",
                .list => "list of strings",
                .build_id => "build ID",
            },
            .values = option.enum_options,
        });

        var system_libraries_list: std.ArrayList(Report.SystemLibrary) = .empty;
        system_libraries_list.ensureUnusedCapacity(gpa, system_libraries.count()) catch @panic("OOM");
        for (system_libraries.keys(), system_libraries.values()) |name, used_by| {
            system_libraries_list.appendAssumeCapacity(.{ .name = name, .used_by = used_by });
        }
        std.mem.sort(Report.SystemLibrary, system_libraries_list.items, {}, struct {
            pub fn lessThan(_: void, lhs: Report.SystemLibrary, rhs: Report.SystemLibrary) bool {
                return std.mem.order(u8, lhs.name, rhs.name) == .lt;
            }
        }.lessThan);

        var system_integrations_list: std.ArrayList(Report.SystemIntegration) = .empty;
        ...
        var used_dependencies_hashes: std.ArrayList([]const u8) = .empty;
        ...
        const env_var = env_map.get("ZIG_EBUILDER_REPORT_LISTEN_PORT") orelse
            std.debug.panic(
                \\Error: listen port for report client is not defined:
                \\ZIG_EBUILDER_REPORT_LISTEN_PORT not set
                \\
            , .{});
        const port = std.fmt.parseInt(u16, env_var, 10) catch |err|
            std.debug.panic("Error when parsing listen port for report client from \"zig build\" runner: {s}", .{@errorName(err)});

        const listen_address: std.Io.net.IpAddress = .{ .ip6 = .loopback(port) };
        ...
        var writer = &conn_writer.interface;

        const json_report_formatter = std.json.fmt(report, .{ .whitespace = .minified });
        writer.print("{f}", .{json_report_formatter}) catch |err|
